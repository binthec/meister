...
class PaidVacation extends Model
{
	...
	public function calcRemainingDays($date_of_entering = null, $base_date = null, $user_id = null)
	{
		...
		while ($this->today > $start_date) {
			...
			$paid_vacations->remaining_days = $paid_vacations->getOriginalPaidVacation($base_date, $start_date); //有給日数を取得：有給取得によって減算されるカラム
			$paid_vacations->original_paid_vacation = $paid_vacations->getOriginalPaidVacation($base_date, $start_date); //有給日数を取得：本来の日数を記憶しておくためのカラム。減算されない。
			$paid_vacations->save();
